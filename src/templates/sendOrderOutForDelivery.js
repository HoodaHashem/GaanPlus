const sendOrderOutForDeliveryData = (order) => {
  let itemsList = "";
  ...
  <head>
    ...
    <title>Order Out for Delivery #${order.id}</title>
    <style>
  ...
  <body>
    <div class="header">
      <h1>ORDER OUT FOR DELIVERY #${order.id}</h1>
    </div>
    ...
    order.userId.lastName
      ...
      <p style="text-align: center;"><strong>Your order is out for delivery!</strong></p>
    </div>
    <div class="footer">
  ...
  `;
  const emailText = `
ORDER OUT FOR DELIVERY #${order.id}
...
export default sendOrderOutForDeliveryData;
